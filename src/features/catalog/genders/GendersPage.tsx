...
import { GenderInput } from './schema'
...
export const GendersPage: React.FC = () => {
  const {
    genders,
    isLoading,
    createGender,
  ...
  const handleCreateOrUpdate = async (data: GenderInput) => {
    try {
      if (editingGender) {
        await updateGender({ id: editingGender.id, body: data })
      ...
      toast.error('Ocurrió un error al guardar el género')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (paginatedGenders.length === 1 && currentPage > 1) {
        setCurrentPage((prev) => prev - 1)
      }
    } catch (e) {
      toast.error('No se pudo eliminar el género, es posible que esté asociado a productos.')
      setDeletingGender(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Administra las opciones de género para la ropa o artículos (ej. Niño, Niña, Unisex).
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-text-muted mt-1 max-w-xs">
              {searchTerm ? 'Prueba ajustando tu búsqueda' : 'Registra tu primer género para comenzar.'}
            </p>
            {!searchTerm && (
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="icon-sm"
                          onClick={() => setDeletingGender(gender)}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-6 border-t border-border-soft mt-4 dark:border-border-soft">
                <span className="text-xs text-text-muted">
                  Página {currentPage} de {totalPages} ({filteredGenders.length} géneros)
                </span>
      ...
      <AlertDialog
        open={deletingGender !== null}
        onOpenChange={(isOpen) => !isOpen && setDeletingGender(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
